...
- Component Design #1: Music Playlist
  - **Description**:
    - To have a music playlist, to be able to add, remove, skip songs. As well to shuffle and recommend similar songs in a similar genre.
  - **Kernel Methods**:
    -addSong() --add song to playlist
    -removeSong() --remove song from playlist
    -getSongs() -- get specfic song name and artist
    -PlaySong() -- play a specfic song
  - **Secondary Methods**:

    -findSong() -- search for song
    -shuffle() -- randomize order of songs
    -playAll() -- playAll songs in specfic order
    -duration() -- show time length of song or of playlist
    -recommened() --if toggled then give reommended songs at end of playlist
  - **Additional Considerations** (*note*: "I don't know" is an acceptable
    answer for each of the following questions):
    - Would this component be mutable? Answer and explain:
    ...
    - Would this component rely on any internal classes (e.g., `Map.Pair`)?
      Answer and explain:
      - Yes it could rely on queue as people may want to play songs in order of placed into a playlist, or it would be randomized. Stack would work in a similar way but queue may be the best choice.
